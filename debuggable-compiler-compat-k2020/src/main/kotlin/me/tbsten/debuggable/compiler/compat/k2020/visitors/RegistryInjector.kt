...
internal fun injectRegistryClose(
    ...
    val allCloseFunctions = irClass.declarations.filterIsInstance<IrSimpleFunction>()
        .filter { fn -> fn.name.asString() == CLOSE_METHOD_NAME && fn.valueParameters.isEmpty() }
    ...
    val closeRegistryFn = symbolProvider.debugCleanupRegistryClass.owner.declarations
        ...
        .firstOrNull { it.name.asString() == CLOSE_METHOD_NAME && it.valueParameters.isEmpty() }
        ?.symbol ?: return
    ...
    val dispatchReceiverParam = closeFunction.dispatchReceiverParameter!!
    ...
    closeFunction.body = builder.irBlockBody {
        +irTry(
            ...
            finallyExpression = irCall(closeRegistryFn).apply {
                dispatchReceiver = irGetField(irGet(dispatchReceiverParam), registryField)
            },
        )
    }
}
