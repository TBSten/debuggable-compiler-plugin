...
| Module (path / Maven artifact ID) | `minVersion` | 対象 patch 範囲 | 代表的な API 境界 |
|---|---|---|---|
| `:debuggable-compiler:compat:k2000` / `debuggable-compiler-compat-k2000` | 2.0.0 | 2.0.0 – 2.0.10 | `BuildersKt` 分離前 |
| `:debuggable-compiler:compat:k2020` / `debuggable-compiler-compat-k2020` | 2.0.20 | 2.0.20 – 2.1.10 | 新 `BuildersKt`、legacy IR builders |
| `:debuggable-compiler:compat:k21` / `debuggable-compiler-compat-k21` | 2.1.20 | 2.1.20 – 2.1.21 | `IrMemberAccessExpression.arguments[param]=` など新 API |
| `:debuggable-compiler:compat:k23` / `debuggable-compiler-compat-k23` | 2.2.0 | 2.2.0 – (最新) | `IrBuilder.irCall`、`IrDeclarationOriginImpl("DEFINED")` |

Gradle project path は `:debuggable-compiler:compat:kXX` (ディレクトリ `debuggable-compiler/compat/kXX/`)。
Maven artifact ID は root `build.gradle.kts` の `project.path.removePrefix(":").replace(":", "-")` で
導出するので `debuggable-compiler-compat-kXX` のまま flat。
...
3. **`build.gradle.kts`** 編集:
   ...
   - `compileOnly(libs.compat.embeddable.k{新名})` — 先に `gradle/libs.versions.toml`
     の `[versions]` と `[libraries]` に `compat-embeddable-k{新名}` を足しておく
     (バージョンはこの module がサポートする最も古い patch)
...
8. **`README.md` / `README.ja.md`** の compat modules テーブルにも行追加 (Maven
   artifact ID は `debuggable-compiler-compat-k{新名}`、root `build.gradle.kts` の
   path→artifactId 変換で自動導出される)。
