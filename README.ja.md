...
`diagram = true` を指定すると、メソッド呼び出し元で各入力変数の値をキャプチャし、[Kotlin Power-Assert](https://kotlinlang.org/docs/power-assert.html) スタイルで並べて出力します。コンパイラが呼び出しサイトを書き換えて、実引数の変数名と具体的な値をインラインコメントとしてログに添付します。
...
// 出力: process(h + f)  // h=123, f=456
```

インラインコメントには呼び出し時点の各リーフ変数とその値が並びます。デバッガなしで「実際に何が渡されたか」を素早く把握できます。

> **Note**: 引数がリテラル定数の場合（例: `Calc.process(42)`）はキャプチャできる変数がなくログは省略されます。`diagram = true` を付けたクラスでは通常の `logAction` ログは出力されず、ダイアグラムログが代わりに記録されます。

### 4. コールスタックのキャプチャ (JVM / Android のみ)

`captureStack = true` を指定すると、メソッド呼び出しのログに呼び出し元のスタックトレースが付加されます。どこから呼ばれたかを素早く特定できます。
...
> **Note**: JVM / Android ターゲットのみ対応です。JS・wasmJs・Native ターゲットでは通常のログ（スタックトレースなし）が出力されます。
